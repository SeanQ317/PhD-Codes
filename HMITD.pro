...
dif=make_array(4096,4096,/double)
...
for imageno = 1, 59 do begin
	...
	xstart = xcenp2 - xsize/2.0
	xend = xcenp2 + xsize/2.0
	ystart = ycenp2 - ysize/2.0
	yend = ycenp2 + ysize/2.0
	...
	while init LE 150 do begin
		...
		for r = 0, 89999 do begin
			if(radius(r) GE init AND radius(r) LT fini AND angl(r) GE -55.0 AND angl(r) LE -35.0) THEN begin
