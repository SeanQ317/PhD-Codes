...
np=lp_get(f[2], 4*((6*11)+1)+0)		;GIVES ARRAY OF [X,Y] AT TIME 6 (Best frame to start with), FOR 0 SPECTRAL POSITION (CHANGE 6 DEPENDING O CASE)
np1=lp_get(f[2], 4*((6*11)+1)+5)	;GIVES ARRAY OF [X,Y] AT TIME (Best frame to start with), FOR MIDDLE SPECTRAL POSITION (CHANGE 6 DEPENDING ON CASE)
...
FOR sc=0, nt-1 DO BEGIN   		;FOR FRAME 0 TO 225 		
  ...
  FOR ii=0, dim[1]-1 DO BEGIN			;FOR X-PIXEL 0 TO 966
	...
	nx=lp_get(f[3], 4.0*((jj*967.0)+ii)+1)		;GIVES AN ARRAY OF [SPECTRAL POSITION, FRAME NUMBER] TO MOVE ALONG EACH PIXEL VIA II AND JJ
  ...
  save,b_cog,v_cog,filename=savepath+'v_cog.idl'			;SAVES B_COG AND V_COG IN SAME DIRECTORY AS START POINT
