...
radius = make_array(401,401, 225,/double)	;RADIUS OF BOX, FOR EACH FRAME (WILL BE THE SAME)
value = make_array(401,401, 225,/double)	;DIFFERENCE DOPPLERGRAM IN BOX, FOR EACH FRAME
td = make_array(201,/double)
tdtbl1 = make_array(201,225,/double)		
angl = make_array(401,401, 225,/double)		;ANGLE QUAKE IS INSIDE BOX, FROM CENTRE OF BOX, FOR EACH FRAME
box = make_array(401,401,225,/double)		;CREATES BOX AROUND DESIRED REGION, FOR ECH FRAME
box1= make_array(401,401,/double)			;CREATES A 2D BOX AROUND DESIRED REGION, FOR EACH FRAME, BUT NOT SAVING THEM
...
FOR t=80, 90 DO BEGIN &$		;MUST BE ONE LESS THAN MAX FRAME, AS CAN'T REMOVES NEXT FRAME AFTER IT
	...
	FOR x=0, 400 do begin
		
		j=0
		
		FOR y=0, 400 do begin
	...
	WHILE init LE 200 DO BEGIN	&$																						;WHILE LOOP 1
		...
		FOR x = 0, 400 DO BEGIN		&$
			FOR y=0, 400 DO BEGIN	 &$
				IF(radius[x,y,t] GE init AND radius[x,y,t] LT fini AND angl[x,y,t] GE 45.0 AND angl[x,y,t] LE 100.0) THEN BEGIN	&$			;IF LOOP 2
...
save,tdtbl1,filename=savepath+'Test_Bot.idl'
