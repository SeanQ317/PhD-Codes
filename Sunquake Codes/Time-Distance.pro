...
radius = make_array(400,400, 134,/double)	;RADIUS OF BOX, FOR EACH FRAME (WILL BE THE SAME)
value = make_array(400,400, 134,/double)	;DIFFERENCE DOPPLERGRAM IN BOX, FOR EACH FRAME
td = make_array(200,/double)
tdtbl1 = make_array(200,134,/double)		
angl = make_array(400,400, 134,/double)		;ANGLE QUAKE IS INSIDE BOX, FROM CENTRE OF BOX, FOR EACH FRAME
box = make_array(400,400,134,/double)		;CREATES BOX AROUND DESIRED REGION, FOR ECH FRAME
box1= make_array(400,400,/double)			;CREATES A 2D BOX AROUND DESIRED REGION, FOR EACH FRAME, BUT NOT SAVING THEM

xstart = 515.0					;START OF BOX IN X-DIRECTION
xend = 915.0					;END OF BOX IN X-DIRECTION
ystart = 280.0					;START OF BOX IN Y-DIRECTION
yend = 680.0					;END OF BOX IN Y-DIRECTION

v_cog_n=v_cog*(-1)			;SWAPS THE INTENSITIES ROUND SO THEY MAKE SENSE 
v_cog_new=v_cog_n(*,*,90:*)

dif=make_array(967, 972, 134)	;MAKES AN ARRAY TO BE FILLED BY FOR LOOP 

r=double(0)
savepath='./'

FOR t=0, 134 DO BEGIN &$		;MUST BE ONE LESS THAN MAX FRAME, AS CAN'T REMOVES NEXT FRAME AFTER IT
