...
l0=6563.000d-10			;FROM PETER VAN HOOF ATOMIC LINE LIST..
c=double(299792000.)
e=-1.6021766d-19
me=9.10938d-31
...
l=13			;NUMBER OF SPECTRAL POSITIONS
...
Ha6563=6563.0							;Caii8542 wavelength (CHNAGE DEPENDING ON SPECTRA)


np=lp_get(f[0], ((6*13)+0)+0)		;GIVES ARRAY OF [X,Y] AT TIME 6 (Best frame to start with), FOR 0 SPECTRAL POSITION (CHANGE 6 DEPENDING O CASE)
np1=lp_get(f[0], ((6*13)+0)+6)	;GIVES ARRAY OF [X,Y] AT TIME (Best frame to start with), FOR MIDDLE SPECTRAL POSITION (CHANGE 6 DEPENDING ON CASE)
;nb=lp_get(f[2], 4*((6*11)+0)+3)		;GIVES ARRAY OF [X,Y] AT TIME (Best frame to start with), FOR BEST SPECTRAL POSITION (CHANGE 6 DEPENDING ON CASE)

wav=Ha6563-spect_pos				;GIVES ARRAY OF [CHANGES IN WAVELENGTH FROM 8542] 
...
path='./'
  ...
  v_cog=dblarr(dim[1],dim[2],nt)			;cog=Center of Gravity (VELOCITY)
  b_cog=dblarr(dim[1],dim[2],nt)								;  (MAGNETIC)

FOR sc=0, nt-1 DO BEGIN   		;FOR FRAME 0 TO 225 		
  ;sc=226
  ...
  FOR ii=0, dim[1]-1 DO BEGIN			;FOR X-PIXEL 0 TO 966
	...
	nx=lp_get(f[1], ((jj*969.0)+ii)+0)		;GIVES AN ARRAY OF [SPECTRAL POSITION, FRAME NUMBER] TO MOVE ALONG EACH PIXEL VIA II AND JJ
...
;	b_cog[ii,jj,sc] = ((lplus - lminus) / 2.) * factor   
  ...
  save,b_cog,v_cog,filename=savepath+'hyd_v_cog.idl'			;SAVES B_COG AND V_COG IN SAME DIRECTORY AS START POINT
